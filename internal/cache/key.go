...
// Uses SHA-2 (SHA-256) for cache key derivation only: same inputs produce the same key for
// lookup/insert. No password or secret is hashed; inputs are tenant id, model name, and
// prompt text. SHA-2 (or SHA-3) is appropriate for non-password uses like cache keys.
//
